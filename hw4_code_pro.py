...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    n_left = np.arange(1, len(sorted_feature))
    n_right = len(sorted_feature) - n_left
    ...
    ginis = -(n_left / len(sorted_feature)) * H_left - (n_right / len(sorted_feature)) * H_right

    valid_indices = np.where((n_left > 0) & (n_right > 0))
